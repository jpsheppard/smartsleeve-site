...
def build_catalog_and_vars_for_all_products(
    ...
    for product in products:
        ...
        append_product_vars(vars_lines, key, name, product_id, prices, sync_variant_ids)
        public_products.append(public_product(key, name, product_id, prices, sync_product, product))
        time.sleep(0.1)
...
def build_catalog_and_vars(
    ...
    for target in targets:
        ...
        if not prices:
            ...
            continue
        append_product_vars(vars_lines, target.key, target.name, product_id, prices, sync_variant_ids)
        public_products.append(public_product(target.key, target.name, product_id, prices, sync_product, matched, target.preview))
        time.sleep(0.1)
