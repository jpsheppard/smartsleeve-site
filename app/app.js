(function () {
  ...
  function accountActiveSleeveNames(account) {
    var names = [];
    ...
    (Array.isArray(account.sleeves) ? account.sleeves : []).forEach(function (sleeve) {
      if (!sleeve || typeof sleeve !== "object") return;
      var name = sleeveLabel(sleeve, "");
      if (!name) return;
      var sleeveHoldings = (sleeve.holdings || []).map(function (holding) {
    ...
    if (!names.length && (!Array.isArray(account.sleeves) || !account.sleeves.some(function (item) { return item && typeof item === "object"; }))) {
      splitSleeves(account.sleeves.length ? account.sleeves : account.sleevesText).forEach(function (name) {
        if (names.indexOf(name) === -1) names.push(name);
      });
    }
    if (!names.length && positions.length && accountDefaultSleeveName(account) !== "Unassigned") {
      names.push(accountDefaultSleeveName(account));
    }
    return names;
  }
  ...
  function normalizeAccount(account) {
    ...
    var accountValueSource = account.accountValueSource || account.account_value_source || account.equitySource || account.equity_source || "broker_reported_equity";
    var accountValueAuthority = account.accountValueAuthority || account.account_value_authority || "";
    ...
    if (accountValueSource === "broker_reported_equity" && equitySource === "broker") {
      equitySource = "broker_reported_equity";
    }
    ...
    return {
      ...
      accountValueDelta: accountValueDelta,
      accountValueSource: accountValueSource,
      accountValueAuthority: accountValueAuthority,
  ...
  function aggregateHoldings(accounts) {
      ...
      account.positions.forEach(function (position) {
        ...
        if (!grouped[symbol]) {
          grouped[symbol] = {
            ...
            quoteSource: "",
            impliedPriceValue: 0,
            impliedPriceShares: 0,
            priceAsOf: null,
            priceSource: "",
            priceDerivedFromMarketValue: false,
            costBasis: null,
            costShares: 0,
            dailyPnl: null,
            unrealizedPnl: null,
        ...
        if (price && shares) {
          ...
          grouped[symbol].priceShares += shares;
        } else if (value && shares) {
          grouped[symbol].impliedPriceValue += value;
          grouped[symbol].impliedPriceShares += shares;
          grouped[symbol].priceDerivedFromMarketValue = true;
        }
        if (position.quotePrice && shares) {
          grouped[symbol].quoteValue += position.quotePrice * shares;
          grouped[symbol].quoteShares += shares;
        }
        if (position.quoteAsOf && (!grouped[symbol].quoteAsOf || timestampMs(position.quoteAsOf) > timestampMs(grouped[symbol].quoteAsOf))) {
          grouped[symbol].quoteAsOf = position.quoteAsOf;
        }
        if (position.quoteSource && grouped[symbol].quoteSource.indexOf(position.quoteSource) === -1) {
          grouped[symbol].quoteSource = grouped[symbol].quoteSource ? grouped[symbol].quoteSource + ", " + position.quoteSource : position.quoteSource;
        }
        var priceAsOf = position.priceAsOf;
        if (priceAsOf && (!grouped[symbol].priceAsOf || timestampMs(priceAsOf) > timestampMs(grouped[symbol].priceAsOf))) {
          grouped[symbol].priceAsOf = priceAsOf;
        }
        if (position.priceSource && grouped[symbol].priceSource.indexOf(position.priceSource) === -1) {
          grouped[symbol].priceSource = grouped[symbol].priceSource ? grouped[symbol].priceSource + ", " + position.priceSource : position.priceSource;
        }
    ...
    var holdings = Object.keys(grouped).map(function (symbol) {
      ...
      row.price = row.priceShares ? row.priceValue / row.priceShares : (row.impliedPriceShares ? row.impliedPriceValue / row.impliedPriceShares : null);
      row.quotePrice = row.quoteShares ? row.quoteValue / row.quoteShares : null;
      row.priceDerivedFromMarketValue = !row.priceShares && row.impliedPriceShares > 0;
  ...
  function marginUsageNote() {
    var margin = marginUsed();
    var buyPower = accountTotal("buyPower");
    if (margin > 0) {
      return "Negative cash detected; review broker maintenance and " + money(buyPower) + " buying-power buffer before adding risk.";
    }
    return "No negative-cash margin detected; buying power is " + money(buyPower) + " across visible accounts.";
  }

  function renderBrokerAccuracyMetric() {
    var value = $("broker-accuracy");
    var note = $("broker-accuracy-note");
    if (!value || !note) return;
    var stale = state.accounts.filter(function (account) { return account.sourceIsStale; });
    var deltas = state.accounts.filter(function (account) {
      var delta = numeric(account.accountValueDelta);
      return delta != null && Math.abs(delta) >= 0.005;
    });
    var diagnostics = state.accounts.filter(function (account) { return accountDiagnosticProblem(account); });
    var missingBrokerValues = state.accounts.filter(function (account) {
      return /robinhood/i.test(account.broker || "") && numeric(account.brokerReportedEquity) == null;
    });
    var totalGaps = stale.length + deltas.length + diagnostics.length + missingBrokerValues.length;
    value.classList.remove("positive", "negative", "neutral", "needs-sync");
    if (!state.accounts.length) {
      value.classList.add("needs-sync");
      value.textContent = "Needs feed";
      note.textContent = "No account rows are visible in this app scope.";
      return;
    }
    if (!totalGaps) {
    ...
    value.classList.add("needs-sync");
    value.textContent = totalGaps + " gap" + (totalGaps === 1 ? "" : "s");
    var parts = [];
    if (deltas.length) parts.push(deltas.length + " reconciliation delta" + (deltas.length === 1 ? "" : "s") + " (" + brokerDeltaSummary(deltas) + ")");
    if (stale.length) parts.push(stale.length + " stale export" + (stale.length === 1 ? "" : "s") + " (" + accountNameSummary(stale) + ")");
    if (diagnostics.length) parts.push(diagnostics.length + " daemon diagnostic" + (diagnostics.length === 1 ? "" : "s") + " (" + accountNameSummary(diagnostics) + ")");
    if (missingBrokerValues.length) parts.push(missingBrokerValues.length + " RH account" + (missingBrokerValues.length === 1 ? "" : "s") + " missing broker value");
    note.textContent = parts.join("; ") + ".";
  }

  function accountNameSummary(accounts) {
    return accounts.slice(0, 3).map(function (account) { return account.account; }).join(", ") + (accounts.length > 3 ? ", +" + (accounts.length - 3) : "");
  }

  function brokerDeltaSummary(accounts) {
    return accounts.slice(0, 2).map(function (account) {
      return account.account + " " + signedMoney(account.accountValueDelta);
    }).join(", ") + (accounts.length > 2 ? ", +" + (accounts.length - 2) : "");
  }
  ...
  function accountDetailHoldingCharts(account, holdings) {
    var rows = (holdings || []).filter(function (position) {
      return numeric(position.shares) > 0 && String(position.symbol || "").trim();
    }).slice(0, 6);
    if (!rows.length) {
      return "<article class=\"panel-card wide-card\"><div class=\"card-head\"><div><span>Holding tape</span><h2>Position price charts</h2></div><span class=\"status-chip\">0 positions</span></div>"
        + emptyItem("No position charts", "Synced holdings are needed before SmartSleeve can draw account-level stock charts.")
        + "</article>";
    }
    return "<article class=\"panel-card wide-card\"><div class=\"card-head\"><div><span>Holding tape</span><h2>Position price charts</h2></div><span class=\"status-chip\">Interactive marks</span></div>"
      + "<div class=\"holding-chart-grid\">"
      + rows.map(function (position) {
        var points = positionPriceHistoryPoints(account, position);
        var title = position.symbol + " / " + account.account;
        var marketTime = shortTimestamp(positionMarketDataTimestamp(position) || accountMarketDataTimestamp(account));
        var meta = (position.name || tickerNames[position.symbol] || position.symbol)
          + " / " + numberText(position.shares, 6) + " sh / " + money(position.value)
          + (marketTime ? " / market data as-of " + marketTime : "");
        return lineChartCard(title, meta, points, "price", "Stock price", position.totalPnl);
      }).join("")
      + "</div></article>";
  }
  ...
  function positionPriceHistoryPoints(account, position) {
    ...
    var rows = (state.history.positions || []).filter(function (point) {
      ...
      var price = numeric(point.price != null ? point.price : point.markPrice != null ? point.markPrice : point.market_price != null ? point.market_price : point.currentPrice != null ? point.currentPrice : point.current_price);
      if (price == null) {
        var shares = numeric(point.shares != null ? point.shares : point.quantity);
        var value = numeric(point.value != null ? point.value : point.marketValue != null ? point.marketValue : point.market_value_usd);
        price = shares ? value / shares : null;
      }
      return {at: point.at || point.timestamp || point.generatedAt || point.generated_at, price: price};
    }).filter(function (point) {
      return point.at && numeric(point.price) != null;
    });
    var latestPrice = numeric(position.quotePrice != null ? position.quotePrice : position.price);
    var latestAt = position.quoteAsOf || position.priceAsOf || account.generatedAt || account.latestGeneratedAt;
    if (latestPrice != null && latestAt) {
      rows.push({at: latestAt, price: latestPrice});
    }
    return dedupeChartPoints(rows, "price");
  }

  function dedupeChartPoints(points, valueKey) {
    var byTime = {};
    (points || []).forEach(function (point) {
      var time = new Date(point.at).getTime();
      var value = numeric(point[valueKey]);
      if (Number.isNaN(time) || value == null) return;
      byTime[String(time)] = {at: new Date(time).toISOString(), value: value};
    });
    return Object.keys(byTime).sort(function (a, b) { return Number(a) - Number(b); }).map(function (key) {
      var row = byTime[key];
      var result = {at: row.at};
      result[valueKey] = row.value;
      return result;
    });
  }
  ...
  function accountHistoryPoints(account) {
    var rows = (state.history.accounts || []).filter(function (row) {
      ...
      if (source === "positions_plus_cash_yfinance"
        && account.accountValueSource === "broker_reported_equity"
  ...
  function accountMarketDataTimestamp(account) {
    ...
    if (quoteTimes.length) {
      return latestTimestamp(quoteTimes);
    }
    var brokerMarkTimes = (account.positions || []).map(function (position) {
      return position.priceAsOf || position.price_as_of;
    }).filter(Boolean);
    if (brokerMarkTimes.length) {
  ...
  function accountMarketDataLabel(account) {
    ...
    return timestamp ? shortTimestamp(timestamp) : "Needs market timestamp";
  }
  ...
  function accountNeedsBrokerReconciliation(account) {
    ...
    var delta = numeric(account.accountValueDelta);
    if (delta != null && Math.abs(delta) >= 0.005) return true;
  ...
  function brokerReconciliationLabel(account) {
    ...
    var delta = numeric(account.accountValueDelta);
    if (delta != null && Math.abs(delta) >= 0.005) return "Delta " + signedMoney(delta);
    var source = String(account.portfolioSource || account.portfolio_source || "").toLowerCase();
  ...
  function brokerReconciliationBody(account) {
    ...
    if (delta != null && Math.abs(delta) >= 0.005) {
      return "App-feed broker reconciliation reports account value minus holdings/cash delta of " + signedMoney(delta) + ". Treat the broker app/account export as ground truth until the next account-scoped sync reconciles to the cent.";
    }
    var source = String(account.portfolioSource || account.portfolio_source || "").toLowerCase();
  ...
  function brokerGroundTruthMetric(account) {
    var brokerValue = numeric(account.brokerReportedEquity != null ? account.brokerReportedEquity : account.brokerEquity);
    var broker = displayLabel(account.broker, "Broker");
    if (brokerValue == null) {
      var warning = /robinhood/i.test(broker) ? "warning" : "";
      return detailMetric("Broker ground truth", "Needs broker value", broker + " did not provide a current account-value field in this app feed. Refresh the account-scoped broker sync before treating the displayed value as exact.", warning);
    }
    var delta = numeric(account.accountValueDelta);
    var body = broker + " account value from the app feed, shown to the nearest cent.";
    if (delta != null) {
      body += " Reconciliation delta versus holdings plus cash: " + signedMoney(delta) + ".";
    }
    return detailMetric("Broker ground truth", money(brokerValue), body, delta != null && Math.abs(delta) >= 0.005 ? "warning" : "");
  }
  ...
  function accountValueSourceCopy(account) {
    if (account.accountValueAuthority) {
      var delta = numeric(account.accountValueDelta);
      var reconciliation = delta != null && Math.abs(delta) >= 0.005
        ? " Holdings plus cash differ by " + signedMoney(delta) + "; broker value remains authoritative."
        : " Holdings plus cash reconcile to the displayed value.";
      return account.accountValueAuthority + "." + reconciliation;
    }
    if (account.equitySource === "broker_reported_equity") {
      return "Broker-reported account value; supplemental quote charts do not override it.";
    }
    if (account.equitySource === "broker_equity") {
      return "Fresh broker equity was preferred.";
    }
    if (account.equitySource === "positions_plus_cash_estimate") {
      return "Positions plus cash estimate.";
    }
  ...
  function priceMarkCell(position) {
    ...
    var quoteAsOf = shortTimestamp(position.quoteAsOf);
    var quote = position.quotePrice == null ? "" : " | " + priceSourceLabel(position.quoteSource || "quote") + " " + money(position.quotePrice) + (quoteAsOf ? " as-of " + quoteAsOf : "");
    return "<b>" + html(money(position.price)) + "</b><small>" + html(source + (asOf ? " as-of " + asOf : "") + quote) + "</small>";
  }
  ...
  function easternTimestamp(value, options) {
    ...
    if (Number.isNaN(date.getTime())) return "";
    return date.toLocaleString("en-US", Object.assign({timeZone: "America/New_York", timeZoneName: "short"}, options || {})).replace(/\bEST\b|\bEDT\b/, "ET");
  }
  ...
  function renderSleeveSummary(total) {
    ...
    target.innerHTML = sleeves.slice(0, 8).map(function (sleeve) {
      ...
      return stackItem(sleeve.name, value, sleeve.accounts.join(", ") + " / " + (sleeve.holdings.slice(0, 7).join(", ") || "No current holdings"));
  ...
  function renderHoldingsTable() {
    ...
    target.innerHTML = rows.map(function (holding) {
      ...
      return "<tr>"
        ...
        + cell("Chart", "<button type=\"button\" class=\"icon-text-button\" data-holding-chart=\"" + html(holding.symbol) + "\">Open chart</button>")
  ...
  function accountIdForHoldingChart(symbol) {
    var normalized = String(symbol || "").toUpperCase();
    var bestAccount = null;
    var bestValue = -Infinity;
    state.accounts.forEach(function (account) {
      (account.positions || []).forEach(function (position) {
        if (String(position.symbol || "").toUpperCase() !== normalized) return;
        var value = numeric(position.value) || 0;
        if (!bestAccount || value > bestValue) {
          bestAccount = account;
          bestValue = value;
        }
      });
    });
    return bestAccount ? String(bestAccount.id) : "";
  }

  function holdingPriceCell(holding) {
    var price = numeric(holding.price);
    if (price == null) {
      return "<span class=\"needs-sync\">Needs quote sync</span>";
    }
    var meta = priceFreshnessLabel(holding);
    var quote = numeric(holding.quotePrice);
    var quoteMeta = quote == null ? "" : "<small>" + html("quote " + money(quote) + " / " + priceFreshnessLabel({
      priceSource: holding.quoteSource || "quote",
      priceAsOf: holding.quoteAsOf
    })) + "</small>";
    return html(money(price)) + (meta ? "<small>" + html(meta) + "</small>" : "") + quoteMeta;
  }

  function priceFreshnessLabel(holding) {
    var parts = [];
    var source = String(holding.priceSource || "").replace(/_/g, " ").trim();
    if (holding.priceDerivedFromMarketValue) {
      parts.push("value-implied mark");
    } else if (source) {
      parts.push(source);
    } else {
      parts.push("broker mark");
    }
    var asOf = holding.priceAsOf || holding.quoteAsOf;
    var ageMs = Date.now() - timestampMs(asOf);
    if (asOf && Number.isFinite(ageMs) && ageMs > 15 * 60 * 1000) {
      parts.push("stale " + ageLabel(ageMs));
    } else if (asOf) {
      parts.push("as-of " + shortTime(asOf));
    }
    return parts.join(" / ");
  }
  ...
  function renderSageReview() {
    ...
    review.innerHTML = [
      stackItem("Current value", money(total), "Tracked account equity in this app scope."),
      stackItem("Largest position", top ? top.symbol + " / " + pct(top.value, total) : "Needs holdings", top ? thesisStatus(top.symbol, top.value / total) : "Connect broker."),
      stackItem("MU plus SNDK", money(semiValue) + " / " + pct(semiValue, total), "Semiconductor pair is the first concentration review target."),
  ...
  function merchGender(product) {
    ...
    return value.indexOf("women") !== -1 ? "Women" : "Men";
  }
  ...
  function merchSortValue(value, order) {
    var index = order.indexOf(value);
    return index === -1 ? order.length : index;
  }

  function merchProductSort(a, b) {
    var genderOrder = ["Men", "Women"];
    var typeOrder = ["T-Shirt", "Tank Top", "Muscle Tee"];
    var frontOrder = ["SS", "SQTS"];
    var backOrder = ["Black", "Website", "Website + QR"];
    return merchSortValue(merchGender(a), genderOrder) - merchSortValue(merchGender(b), genderOrder)
      || merchSortValue(merchCut(a), typeOrder) - merchSortValue(merchCut(b), typeOrder)
      || merchSortValue(merchFrontLabel(a), frontOrder) - merchSortValue(merchFrontLabel(b), frontOrder)
      || merchSortValue(merchBackType(a), backOrder) - merchSortValue(merchBackType(b), backOrder)
      || String(a.name || "").localeCompare(String(b.name || ""));
  }

  function merchGroupKey(product) {
    if (merchCut(product) === "Muscle Tee") return "All::Muscle Tee";
    return merchGender(product) + "::" + merchCut(product);
  }

  function merchGroupSortKey(key) {
    ...
    return groupOrder[key] == null ? 99 : groupOrder[key];
  }

  function merchGroupTitle(key) {
    var parts = String(key || "").split("::");
  ...
  function renderMerchCatalog() {
    ...
    if (!grid) return;
    var products = (state.merchProducts || []).slice().sort(merchProductSort);
    text("merch-catalog-status", products.length ? products.length + " items" : "Loading");
    if (!products.length) {
      grid.innerHTML = emptyItem("Loading shop", " ");
      return;
    }
    var groups = {};
    products.forEach(function (product) {
      var key = merchGroupKey(product);
      if (!groups[key]) groups[key] = [];
      groups[key].push(product);
    });
    grid.innerHTML = Object.keys(groups).sort(function (a, b) {
      return merchGroupSortKey(a) - merchGroupSortKey(b) || a.localeCompare(b);
    }).map(function (key) {
      return "<section class=\"merch-product-group\"><h3>" + html(merchGroupTitle(key)) + "</h3><div class=\"merch-product-row\">"
  ...
  function addMerchToCart(productKey) {
    ...
    text("merch-cart-note", "Shipping and taxes are calculated at checkout.");
  }
  ...
  function loadMerchCatalog() {
    ...
    function mergeCatalogProducts(primary, fallback) {
      var previewByKey = {};
      productsFrom(fallback).forEach(function (product) {
        if (product.preview) previewByKey[product.key] = product.preview;
      });
      return productsFrom(primary).map(function (product) {
        if (!product.preview && previewByKey[product.key]) {
          return Object.assign({}, product, {preview: previewByKey[product.key]});
        }
        return product;
      });
    }
    function applyProducts(products, source) {
      state.merchProducts = products;
    ...
    var fallback = fetch(fallbackUrl, {cache: "no-store"}).then(function (response) {
    ...
    Promise.allSettled([live, fallback]).then(function (results) {
      ...
      if (fallbackPayload) {
        applyProducts(mergeCatalogProducts(fallbackPayload, livePayload), livePayload ? "live" : "snapshot");
      } else if (livePayload) {
        applyProducts(productsFrom(livePayload), "live");
      } else {
        throw results[0].reason || results[1].reason || new Error("Catalog unavailable");
      }
    }).catch(function () {
      text("merch-catalog-status", "Unavailable");
      var grid = $("merch-product-grid");
      if (grid) grid.innerHTML = emptyItem("Shop unavailable", "Please try again soon.");
    });
  }
  ...
  function stackItem(title, meta, body, progress, className) {
    ...
    var compact = classes.split(/\s+/).indexOf("compact-stack") !== -1;
    return "<article class=\"stack-item " + html(classes) + "\"" + (compact && body ? " title=\"" + html(body) + "\"" : "") + "><div class=\"stack-item-head\"><b>" + html(title) + "</b><span>" + html(meta) + "</span></div>"
      + (compact ? "<p class=\"assistive-copy\">" + html(body) + "</p>" : "<p>" + html(body) + "</p>")
      + (showProgress ? "<div class=\"progress-bar\" style=\"--value:" + Math.max(0, Math.min(100, Number(progress) || 0)) + "%\"><i></i></div>" : "")
      + "</article>";
  }

  function detailMetric(title, meta, body, className) {
    var detail = [title, meta, body].filter(Boolean).join(": ");
    return "<article class=\"detail-metric " + html(className || "") + "\" title=\"" + html(detail) + "\" aria-label=\"" + html(detail) + "\">"
      + "<span>" + html(title) + "</span><b>" + html(meta) + "</b>"
      + "</article>";
  }
  ...
  function targetTimingPlan(symbol) {
    ...
    var deadlineLabel = deadline ? easternTimestamp(deadline, {month: "short", day: "numeric", hour: "numeric", minute: "2-digit"}) : "No deadline set";
    if (!history.length) {
      return {
  ...
  function handleNav(section) {
    ...
    document.body.classList.toggle("public-shop", target === "shop");
    ...
    document.title = target === "shop" ? "Merch Shop | SmartSleeve" : "SmartSleeve Portfolio OS";
  ...
  function wireEvents() {
    ...
    document.addEventListener("click", function (event) {
      ...
      var holdingChartButton = event.target.closest("[data-holding-chart]");
      if (holdingChartButton) {
        var accountId = accountIdForHoldingChart(holdingChartButton.getAttribute("data-holding-chart"));
        if (accountId) {
          state.selectedDetailAccountId = accountId;
          renderAccountDetail();
          handleNav("account-detail");
          history.replaceState(null, "", "#account-detail");
          toast("Opened the account chart for " + holdingChartButton.getAttribute("data-holding-chart") + ".");
        } else {
          toast("No account chart is available for that holding yet.");
        }
        return;
      }
  ...
  function latestDaemonLabel(payload) {
    ...
    return "Last synced trader cycle at " + easternTimestamp(latest, {hour: "2-digit", minute: "2-digit", second: "2-digit", month: "long", day: "numeric", year: "numeric"}) + (staleCount ? " / " + staleCount + " stale account" + (staleCount === 1 ? "" : "s") + "." : ".");
  }
  ...
  function feedSyncLabel(payload) {
    ...
    var base = date && !Number.isNaN(date.getTime())
      ? "App feed refreshed at " + easternTimestamp(date, {hour: "2-digit", minute: "2-digit", second: "2-digit", month: "long", day: "numeric", year: "numeric"}) + "."
